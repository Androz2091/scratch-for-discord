...
const blockData = {
    "message0": "Button Edit %1 Ephemeral %2 Button %3",
    "args0": [
        {
            "type": "input_value",
            "name": "CONTENT",
...
Blockly.JavaScript[blockName] = function(block){
    ...
    if(block.getInput("CONTENT").connection.targetConnection){
        ...
        if((contentType === "Embed") || (!contentType && typeof contentType === "object")){
            const code = `await interaction.update({ ephemeral: ${boolean}, embeds: [${content}], components: [${button}] });\n`;
            return code;
        } else {
            const code = `await interaction.update({ ephemeral: ${boolean}, content: ${content}, components: [${button}] });\n`;
            return code;
        }
    } else {
        const code = `await interaction.update({ ephemeral: ${boolean}, content: ${content}, components: [${button}] });\n`;
